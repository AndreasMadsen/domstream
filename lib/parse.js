...
//  that pos[1] is relative to pos[0], reduce calculations
//  that pos[3] is relative to pos[2], reduce calculations
//  that attr.start is relative to last attr.start or pos[0];
...
function parse(content) {
  ...
  var root = {
    isRoot: true,
    pos: [ 0, begin - 0, end, (content.length - 1) - end ],
...
function endTag(elem, tag) {
  elem.pos[2] = tag.start;
  elem.pos[3] = tag.end - tag.start;
}

function createTag(content, tag, parent) {
  // this should have the same order as in copy.js
  var elem = {
    pos: [ tag.start, tag.end - tag.start, -1, -1 ],
