...
Document.prototype.resume = function () {
  ...
  if (current !== next) {
    this.emit('data', this.content.slice(current, next));
  }

  if (next === this.tree.pos[2] + 1) {
    this.readable = false;
    this.emit('end');
    this.emit('close');
  }
...
Document.prototype.pause = function () {
  this.paused = true;
};
...
Document.prototype._send = function (node) {
  ...
  // send chunk if the stream is online
  if (!this.paused) {
    this.resume();
  }
};
...
Document.prototype.container = function (nodes) {
  ...
  loop:while (i--) {
    ...
    pos = {
      start: node.pos[0],
      end: node.singleton ? (node.pos[0] + node.pos[1]) : node.pos[2]
    };
    ...
    while (j--) {
      ...
      if (check.pos[0] < pos.start && check.pos[2] > pos.end) {
        continue loop;
      }
    }
  ...
  topLevel = topLevel.sort(function sortfunction(a, b){
    return (a.elem.pos[0] - b.elem.pos[0]);
  });
  ...
  // send first chunk
  this.nextPos = topLevel[0].elem.pos[0];
